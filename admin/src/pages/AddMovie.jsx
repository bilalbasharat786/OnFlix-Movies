...
const AddMovie = () => {
  ...
  const autoFetchTop100 = async () => {
    const confirmImport = window.confirm("Kya aap waqai 2023 ki Top 100 Bollywood movies automatically add karna chahte hain? Isme 1-2 minute lag sakte hain!");
    ...
    try {
      ...
      for (let page = 1; page <= 5; page++) {
        ...
        // TMDB Discover API: Language=hi (Hindi), Year=2023
        const discoverUrl = `https://api.themoviedb.org/3/discover/movie?api_key=${TMDB_API_KEY}&with_original_language=hi&primary_release_year=2023&sort_by=popularity.desc&page=${page}`;
        ...
        for (const tmdbMovie of moviesList) {
          try {
            ...
            const releaseYear = tmdbMovie.release_date ? tmdbMovie.release_date.split('-')[0] : 2023;
  ...
  return (
    <div className="max-w-2xl mx-auto bg-gray-800 p-8 rounded-lg shadow-lg border border-gray-700">
      ...
      <div className="mb-8 p-4 bg-gray-900 border-2 border-dashed border-red-500 rounded-lg text-center">
        ...
        <p className="text-sm text-gray-400 mb-4">Click karte hi 2023 ki top 100 Bollywood movies aapke database mein khud add ho jayengi!</p>
        <button
          type="button"
        ...
        >
          {bulkLoading ? `Rukiye... ${bulkProgress}` : "🚀 Auto-Fetch Top 100 Bollywood Movies (2023)"}
        </button>
      </div>
